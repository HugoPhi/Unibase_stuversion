...
int IxNodeHandle::lower_bound(const char *target) const {
    ...
    {
        int mid = (l + r) >> 1;
        const char *chptr = get_key(mid);
        if(ix_compare(chptr, target, this->file_hdr->col_types_, this->file_hdr->col_lens_) >= 0) 
            r = mid;
        else 
            l = mid + 1;
    }
...
bool IxNodeHandle::leaf_lookup(const char *key, Rid **value) {
    ...
    int klocation = lower_bound(key);
    if(klocation == get_size() || ix_compare(key, get_key(klocation), this->file_hdr->col_types_, this->file_hdr->col_lens_) != 0) return false;
    *value = get_rid(klocation);
    return true;
}
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    for( int i = 0; i < n; i ++ ) {
        set_key(pos + i, key + file_hdr -> col_lens_[i] * i );  // ?????
        set_rid(pos + i, rid[i]);
    }
    set_size( num + n );
}
...
int IxNodeHandle::insert(const char *key, const Rid &value) {
    ...
    if( ix_compare( key, get_key(location), file_hdr -> col_types_, file_hdr -> col_lens_ ) != 0 ) {
...
std::pair<IxNodeHandle *, bool> IxIndexHandle::find_leaf_page(const char *key, Operation operation,
    ...
    IxNodeHandle *node = this->fetch_node(this->file_hdr_->root_page_);
    assert( node != nullptr );
    while( !node -> is_leaf_page() ) {
        node = this->fetch_node(node -> internal_lookup(key));
...
bool IxIndexHandle::get_value(const char *key, std::vector<Rid> *result, Transaction *transaction) {
    ...
    auto leaf = this->find_leaf_page(key, Operation::FIND, transaction).first;
    
    Rid *rid;
    bool flag = leaf->leaf_lookup(key, &rid);
...
// 初始化辅助函数
void IxIndexHandle::__init__(IxNodeHandle *node) {
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    auto sibling = create_node(); 
    this->__init__(sibling);

    if( node -> is_leaf_page() ) {
        sibling -> page_hdr -> is_leaf = true;
        
        auto nxt = fetch_node( node -> get_next_leaf() );
        if( node -> get_page_no() == this->file_hdr_->last_leaf_ ) {
            this->file_hdr_->last_leaf_ = sibling -> get_page_no();
        }

        sibling -> set_next_leaf( node -> get_next_leaf() );
        nxt -> set_prev_leaf( sibling -> get_page_no() );

        sibling -> set_prev_leaf( node -> get_page_no() );
        node -> set_next_leaf( sibling -> get_page_no() );

        buffer_pool_manager_ -> unpin_page(nxt -> get_page_id(), true);
    }

    int pos = node -> get_size() / 2;
    sibling -> insert_pairs(0, node -> get_key(pos), node -> get_rid(pos), (node -> get_size() + 1) / 2);
    node -> set_size(pos);
    for( int i = 0; i < sibling -> get_size(); ++i ) maintain_child(sibling, i);
...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    if( old_node -> is_root_page() ) {
        auto newroot = create_node(); 
        __init__(newroot);

        file_hdr_->root_page_ = newroot -> get_page_no();
        newroot -> insert_pair(0, old_node -> get_key(0), Rid{old_node -> get_page_no(), -1} );
        old_node -> set_parent_page_no( newroot -> get_page_no() );
        fa = newroot; pos = 0;
    } else fa = fetch_node( old_node -> get_parent_page_no() ), pos = fa -> find_child(old_node);
    fa -> insert_pair(pos + 1, key, Rid{new_node -> get_page_no(), -1} );
    new_node -> set_parent_page_no( fa -> get_page_no() );

    if( fa -> get_size() >= fa -> get_max_size() ) {
        auto fa_bro = split(fa);
        insert_into_parent(fa, fa_bro -> get_key(0), fa_bro, transaction);
        buffer_pool_manager_ -> unpin_page( fa_bro -> get_page_id(), true );
    }
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    int pre = leaf.first -> get_size();
    bool flag = leaf.first->remove(key) != pre;

    maintain_parent(leaf.first);

    if(flag) coalesce_or_redistribute(leaf.first, transaction);
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    std::scoped_lock lock{root_latch_};
    auto leaf = this->find_leaf_page(key, Operation::DELETE, transaction).first;

    int pre = leaf->get_size();
    bool flag = leaf -> remove(key) != pre;

    maintain_parent(leaf);

    // if(flag) CoalesceOrRedistribute(leaf, transaction);
    if(flag) coalesce_or_redistribute(leaf);
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    if( node -> get_size() >= node -> get_min_size() ) return false;

    auto fa  = fetch_node( node -> get_parent_page_no() );
    auto bro = fetch_node( fa -> find_child(node) ? node -> get_prev_leaf() : node -> get_next_leaf() );

    bool flag = node -> get_size() + bro -> get_size() >= 2 * node -> get_min_size();
    if( flag ) redistribute(bro, node, fa, fa -> find_child(node));
    else coalesce(&bro, &node, &fa, fa -> find_child(node), transaction, root_is_latched);

    buffer_pool_manager_ -> unpin_page( fa -> get_page_id(), true );
    buffer_pool_manager_ -> unpin_page( bro -> get_page_id(), true );
    return flag;
    return false;
}
...
bool IxIndexHandle::adjust_root(IxNodeHandle *old_root_node) {
    ...
    if( old_root_node -> is_leaf_page() ) {
        file_hdr_->root_page_ = old_root_node -> get_page_no();
        return false;
    } 
    if( old_root_node -> is_root_page() and old_root_node -> get_size() == 1 ){
...
void IxIndexHandle::redistribute(IxNodeHandle *neighbor_node, IxNodeHandle *node, IxNodeHandle *parent, int index) {
    ...
    int size_node = node->get_size();
    int size_bro = neighbor_node->get_size();
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    //std::cout << "[Index] merge node<" << (*neighbor_node) -> get_page_no() << "> and node<" << (*node) -> get_page_no() << ">\n";

    (*neighbor_node)->insert_pairs((*neighbor_node)->get_size(), (*node)->keys, (*node)->rids, (*node)->get_size());

    for (int i = 1; i <= (*node)->get_size(); i++) {
        maintain_child((*neighbor_node), (*neighbor_node)->get_size() - i);
    }
